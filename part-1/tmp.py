...
def count_dict_values(dict):
    """
    1. 遍历dict，获取value，遍历value，遍历value构成的list，然后统计每个元素出现的次数，加入到一个新的dict中。
    :param dict: the dictionary to be counted.
    :return: dict: key is the same while the value is the count of the same value.
    """
    count_dict = {}
    for key in dict:
        value = dict[key]
...
# test case for count_dict_values()
# print(count_dict_values({'Alice': [20], 'Bob': [25], 'Charlie': [30]}))


nom_count_dict = count_dict_values(nominated)
...
win_count_dict = count_dict_values(winners)
print("win_count_dict = {}\n".format(win_count_dict))

# control_flow_q2
"""
1. 遍历winners，获取value，遍历value，遍历value构成的list，然后统计每个元素出现的次数，加入到一个新的dict中
2. 对新的dict进行排序，取出最大的value，然后遍历新的dict，找到value等于最大value的key，加入到一个list中。
"""
winners = {1931: ['Norman Taurog'], 1932: ['Frank Borzage'], 1933: ['Frank Lloyd'], 1934: ['Frank Capra'],
           1935: ['John Ford'], 1936: ['Frank Capra'], 1937: ['Leo McCarey'], 1938: ['Frank Capra'],
           1939: ['Victor Fleming'], 1940: ['John Ford'], 1941: ['John Ford'], 1942: ['William Wyler'],
...
max_value = 0
# Add your code here
count_dict = count_dict_values(winners)

max_value = max(count_dict.values())
most_win_director = [key for key, value in count_dict.items() if value == max_value]
